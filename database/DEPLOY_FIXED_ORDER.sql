-- NORDFLYTT COMPLETE SCHEMA DEPLOYMENT (FIXED ORDER)
-- Run this in Supabase SQL Editor

-- Step 1: Enable extensions
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- Step 2: Create update function FIRST (before any triggers)
...
-- Step 3: Create all tables WITHOUT triggers first
...
-- Offentliga Upphandlingar (Public Procurements) - WITH category column from start
...
-- Lager/Inventory - WITH category column from start
...
-- Leverantörer/Suppliers - WITH category column from start
...
-- Rapporter/Reports Configuration - WITH category column from start
...
-- Step 5: Create triggers AFTER tables exist
DO $$ 
BEGIN
  -- Only create trigger if it doesn't exist
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_public_procurements_updated_at') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_customer_storage_updated_at') THEN
...
-- Success message
SELECT 'Schema deployment complete! All tables created successfully.' as message;