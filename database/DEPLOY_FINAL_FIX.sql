-- NORDFLYTT SCHEMA DEPLOYMENT - COMPLETE FIX
...
-- Step 3: Fix existing inventory_items table structure
...
-- Add unique constraint on item_code if it doesn't exist
DO $$ 
BEGIN
  IF NOT EXISTS (
...
-- Step 5: Create indexes (skip if column doesn't exist)
...
-- Only create inventory indexes if columns exist
DO $$ 
BEGIN
...
-- Step 7: Insert sample data (only if columns exist)
DO $$ 
BEGIN
  -- Check if all required columns exist before inserting
  IF EXISTS (SELECT 1 FROM information_schema.columns 
             WHERE table_name = 'inventory_items' 
             AND column_name IN ('item_code', 'name', 'description', 'category', 'unit', 'current_stock', 'minimum_stock', 'unit_cost')) THEN
...
SELECT 'Schema deployment complete! All tables and columns have been created/updated.' as message;