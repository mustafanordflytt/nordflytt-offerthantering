...
CREATE TABLE IF NOT EXISTS employee_time_reports (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  employee_id UUID REFERENCES employees(id) ON DELETE CASCADE,
  job_id UUID, -- Optional reference to jobs if table exists
  date DATE NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_assets_employee ON employee_assets(employee_id);
CREATE INDEX IF NOT EXISTS idx_onboarding_employee ON employee_onboarding(employee_id);
CREATE INDEX IF NOT EXISTS idx_time_reports_employee ON employee_time_reports(employee_id);
CREATE INDEX IF NOT EXISTS idx_time_reports_date ON employee_time_reports(date);
...
-- Allow all operations for now (you can refine this later)
CREATE POLICY "Allow all for employees" ON employees
  FOR ALL USING (true);

CREATE POLICY "Allow all for employee_skills" ON employee_skills
  FOR ALL USING (true);

CREATE POLICY "Allow all for employee_contracts" ON employee_contracts
  FOR ALL USING (true);

CREATE POLICY "Allow all for employee_assets" ON employee_assets
  FOR ALL USING (true);

CREATE POLICY "Allow all for employee_onboarding" ON employee_onboarding
  FOR ALL USING (true);

CREATE POLICY "Allow all for employee_vehicle_access" ON employee_vehicle_access
  FOR ALL USING (true);

CREATE POLICY "Allow all for employee_time_reports" ON employee_time_reports
  FOR ALL USING (true);
