// CRM Authentication System with Supabase - FIXED VERSION
...
export class CRMAuthService {
  ...
  async refreshSession(): Promise<{ success: boolean; user?: CRMUser }> {
    try {
      ...
      let refreshedProfile = profileData

      if (profileError || !refreshedProfile) {
      ...
      const crmUser: CRMUser = {
        ...
        permissions: ROLE_PERMISSIONS[refreshedProfile.role],
        department: refreshedProfile.department,
        avatar: refreshedProfile.avatar,
        createdAt: new Date(refreshedProfile.created_at),
        lastLoginAt: refreshedProfile.last_login_at ? new Date(refreshedProfile.last_login_at) : undefined
      }
  ...
  async createUser(userData: {
    ...
    try {
      ...
      const newUser: CRMUser = {
        ...
        permissions: ROLE_PERMISSIONS[newProfileData.role],
        department: newProfileData.department,
        createdAt: new Date(newProfileData.created_at)
      }
